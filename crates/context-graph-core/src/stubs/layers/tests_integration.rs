...
async fn test_pipeline_all_fail_fast() {
    ...
    let r2 = memory.process(input.clone()).await;
    let r3 = learning.process(input.clone()).await;
    let r4 = coherence.process(input.clone()).await;

    assert!(r1.is_err(), "Sensing must fail");
    assert!(r2.is_err(), "Memory must fail");
    assert!(r3.is_err(), "Learning must fail");
    assert!(r4.is_err(), "Coherence must fail");
