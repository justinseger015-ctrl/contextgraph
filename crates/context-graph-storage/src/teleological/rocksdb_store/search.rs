...
impl RocksDbTeleologicalStore {
    ...
    /// Original E1-only search (backward compatible) - DEPRECATED, use search_e1_only_sync.
    ///
    /// This method is kept for internal use by apply_full_temporal_boosts.
    /// Main search flow uses search_e1_only_sync via spawn_blocking.
    #[allow(dead_code)]
    async fn search_e1_only(
        ...
        tokio::task::spawn_blocking(move || {
            search_e1_only_sync(&db, &index_registry, &soft_deleted, &query, &options)
        })
        .await
        .map_err(|e| CoreError::Internal(format!("spawn_blocking failed: {}", e)))?
    }
